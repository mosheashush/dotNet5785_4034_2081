...
internal static class VolunteerManager
{
    ...
    public static BO.CallInProgress? MPIdVolunteerToCallInProgress(int id)
    {
        DO.Assignment assignment = s_dal.Assignment.ReadAll()
            .FirstOrDefault(c => c.VolunteerId == id && c.FinishType == null);

        // תיקון: אם assignment הוא NULL - אין קריאה בטיפול
        if (assignment == null)
            return null;

        // רק אם assignment קיים - ממשיכים
        DO.Call call = s_dal.Call.ReadAll()
            .Where(c => c.Id == assignment.CallId)
            .FirstOrDefault();
        ...
        return new BO.CallInProgress()
        {
            ...
            DistanceFromVolunteer = GetDistanceInKm(
                call.Latitude,
                call.Longitude,
                volunteer.Latitude,
                volunteer.Longitud
